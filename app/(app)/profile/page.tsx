          ...
          <CardContent className="space-y-3">
            {favorites.length === 0 ? (
              ...
              favorites.map((favorite: (typeof favorites)[number]) => (
                <div key={favorite.id} className="flex items-center justify-between gap-3 rounded-md border border-border p-3">
                  <div className="min-w-0">
                    <p className="truncate font-semibold">{favorite.school.name}</p>
                    <p className="truncate text-xs text-muted-foreground">{favorite.school.city}, {favorite.school.state}</p>
                  </div>
